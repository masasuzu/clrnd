...
| the secret **version** each reference points at | Secret Manager `secrets.versions.get` | `latest` is what Cloud Run assumes when no version is given; skipped when the secret itself is missing |
...
| `run.googleapis.com/cloudsql-instances` | Cloud SQL Admin `instances.get` | each `<project>:<region>:<instance>`; the project comes from the connection name, so a cross-project instance works |
